...
export default function RepTypeManager() {
  const [topics, setTopics] = useState([]);
  const [loading, setLoading] = useState(false);
  const [topicModalOpen, setTopicModalOpen] = useState(false);
  const [editTopic, setEditTopic] = useState(null);
  const [topicForm] = Form.useForm();
  const [topicFilters, setTopicFilters] = useState({ repTypeName: null });
  ...
  const handleDeleteTopic = async (id) => {
    ...
    try {
      const response = await fetch(`/api/reptype?id=${id}`, {
        method: 'DELETE',
      });
      const result = await response.json();
      if (result.success) {
  ...
  const resetAllTopicFilters = () => {
    setTopicFilters({ repTypeName: null });
    setTopicSearchText('');
    setTopicSearchedColumn('');
  };
