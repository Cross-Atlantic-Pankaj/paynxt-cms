...
export default function DelManager() {
    ...
    const fetchOurStrengths = async () => {
        try {
            setLoading(true);
            const response = await fetch('/api/reports/repdel');
    ...
    const handleOurStrengthSubmit = async (values) => {
        ...
        try {
            ...
            const sectionsToSend = sections.map((section, index) => {
                ...
                // handle chart
                if (section.chart) {
                    if (section.chart.file) {
                        formData.append(`sections[${index}].chart`, section.chart.file.originFileObj);
                    } else if (Array.isArray(section.chart) && section.chart.length > 0) {
                        if (section.chart[0].url) {
                            chartValue = section.chart[0].url;
                        } else if (section.chart[0].originFileObj) {
                            formData.append(`sections[${index}].chart`, section.chart[0].originFileObj);
                        }
                    }
                }
                ...
                if (!chartValue && !formData.has(`sections[${index}].chart`)) {
                    throw new Error(`Chart for section ${index} is required`);
                }
    ...
    const handleDeleteOurStrength = async (id) => {
        ...
        try {
            const response = await fetch(`/api/reports/repdel?id=${id}`, {
                method: 'DELETE',
            });
            const result = await response.json();
            if (result.success) {
                                ...
                                >
                                    <Space direction="vertical" className="w-full">
                                        ...
                                        <div>
                                            ...
                                            <Image src={section.chart} width={60} height={60} className="object-cover rounded" />
                                        </div>
                                        <div>
                        ...
                        <Button
                            ...
                            onClick={() => {
                                ...
                                const formSections = record.sections.map(section => ({
                                    ...
                                    chart: section.chart ? [{ url: section.chart, uid: section.chart, name: 'chart' }] : [],
                                    imageTitle: section.imageTitle,
                                    description: section.description,
                                }));
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-semibold text-gray-800">Research Deliverables</h2>
                <Space>
            ...
            {canEdit && (
                <Modal
                    title={editOurStrength ? 'Edit Deliverable' : 'Add New Deliverable'}
                                    ...
                                    {fields.map(({ key, name, ...restField }) => (
                                        ...
                                        >
                                            ...
                                            <Form.Item
                                                {...restField}
                                                name={[name, 'chart']}
                                                label={<Text strong>Chart</Text>}
                                                valuePropName="fileList"
                                                getValueFromEvent={normFile}
                                                rules={[{ required: true, message: 'Please upload a chart' }]}
                                            >
                                                <Upload
                                                    beforeUpload={() => false}
                                                    listType="picture-card"
                                                ...
                                                >
                                                    <div className="flex flex-col items-center">
                                                        <UploadOutlined className="text-2xl text-blue-500" />
                                                        <div className="mt-2">Upload Chart</div>
                                                    </div>
                                                </Upload>
                                            </Form.Item>
                                            {ourStrengthForm.getFieldValue(['sections', name, 'chart']) &&
                                                ourStrengthForm.getFieldValue(['sections', name, 'chart']).length > 0 &&
                                                ourStrengthForm.getFieldValue(['sections', name, 'chart'])[0].url && (
                                                    <Form.Item label={<Text strong>Current Chart</Text>}>
                                                        <Image
                                                            src={ourStrengthForm.getFieldValue(['sections', name, 'chart'])[0].url}
                                                            alt="Current"
                                                            width={120}
                                                            height={120}
                                                            className="object-cover rounded"
