...
export default function AdvManager() {
  ...
  const fetchOurStrengths = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/reports/repadv');
  ...
  const handleDeleteOurStrength = async (id) => {
    ...
    try {
      const response = await fetch(`/api/reports/repadv?id=${id}`, {
        method: 'DELETE',
      });
      const result = await response.json();
      if (result.success) {
  ...
  return (
    <div>
      ...
      {canEdit && (
        <Modal
          title={editOurStrength ? 'Edit Advantage' : 'Add New Advantage'}
