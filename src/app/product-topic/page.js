...
export default function ProductTopicManager() {
  const [topics, setTopics] = useState([]);
  const [loading, setLoading] = useState(false);
  const [topicModalOpen, setTopicModalOpen] = useState(false);
  const [editTopic, setEditTopic] = useState(null);
  const [topicForm] = Form.useForm();
  const [topicFilters, setTopicFilters] = useState({ productTopicName: null });
  ...
  const handleDeleteTopic = async (id) => {
    ...
    try {
      const response = await fetch(`/api/product-topic?id=${id}`, {
        method: 'DELETE',
      });
      const result = await response.json();
      if (result.success) {
  ...
  const resetAllTopicFilters = () => {
    setTopicFilters({ productTopicName: null });
    setTopicSearchText('');
    setTopicSearchedColumn('');
  };
  ...
  const topicColumns = [
    {
      title: 'Topic Name',
      dataIndex: 'productTopicName',
      key: 'productTopicName',
      ...getColumnSearchProps('productTopicName'),
    },
    {
  ...
  return (
    <div className="mb-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-2xl font-semibold">Product Topics</h2>
        <div className="flex gap-2">
