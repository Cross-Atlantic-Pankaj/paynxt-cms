...
import Consultsec from '@/models/reports/repconsult';
...
export async function POST(req) {
  try {
    ...
    let platformSection;
    if (body._id) {
      platformSection = await Consultsec.findByIdAndUpdate(
        body._id,
        {
          title: body.title,
      ...
      platformSection = new Consultsec({
        title: body.title,
        description: body.description,
        clickText: body.clickText,
        url: body.url,
...
export async function GET() {
  try {
    await connectDB();
    const platformSections = await Consultsec.find().sort({ createdAt: -1 });
    return NextResponse.json({
      success: true,
...
export async function DELETE(req) {
  try {
    ...
    await Consultsec.findByIdAndDelete(id);
    return NextResponse.json({
      success: true,
