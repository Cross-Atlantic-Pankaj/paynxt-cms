...
export async function POST(req) {
  try {
    ...
    let platformSection;
    if (body._id) {
      platformSection = await PlatformSection.findByIdAndUpdate(
        body._id,
        {
          title: body.title,
      ...
      platformSection = new PlatformSection({
        title: body.title,
        description: body.description,
        clickText: body.clickText,
        url: body.url,
...
export async function GET() {
  try {
    await connectDB();
    const platformSections = await PlatformSection.find().sort({ createdAt: -1 });
    return NextResponse.json({
      success: true,
...
export async function DELETE(req) {
  try {
    ...
    await PlatformSection.findByIdAndDelete(id);
    return NextResponse.json({
      success: true,
