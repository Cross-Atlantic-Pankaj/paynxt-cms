...
import Consult from '@/models/blog-page/consult';
...
export async function POST(req) {
  try {
    ...
    let platformSection;
    if (body._id) {
      platformSection = await Consult.findByIdAndUpdate(
        body._id,
        {
          title: body.title,
...
export async function GET() {
  try {
    await connectDB();
    const platformSections = await Consult.find().sort({ createdAt: -1 });
    return NextResponse.json({
      success: true,
