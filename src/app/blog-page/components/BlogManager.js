...
import { Table, Button, Modal, Form, Input, message, Popconfirm, Tooltip, Upload, Collapse, Tag, DatePicker, Select } from 'antd';
...
export default function BlogManager() {
    ...
    const [selectedCategory, setSelectedCategory] = useState([]); // array, not single string
    const [topics, setTopics] = useState([]);
    const [subtopics, setSubtopics] = useState([]);
    const [selectedTopic, setSelectedTopic] = useState([]); // make it array
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                const res = await fetch('/api/product-category'); // adjust if your route is different
        ...
        const fetchSubcategories = async () => {
            try {
                const res = await fetch('/api/product-subcategory'); // adjust if different
                const data = await res.json();
        ...
        const fetchTopics = async () => {
            try {
                const res = await fetch('/api/product-topic');  // adjust if your path differs
                const data = await res.json();
        ...
        const fetchSubtopics = async () => {
            try {
                const res = await fetch('/api/product-subtopic'); // adjust if your path differs
                const data = await res.json();
    ...
    const handleBlogSubmit = async (values) => {
        ...
        try {
            ...
            // Handle image
            // Handle image properly when editing
            if (values.imageFile && values.imageFile.length > 0) {
                ...
                if (fileObj.originFileObj) {
                    ...
                    // existing image url from previous edit
        ...
        <div>
            <div className="flex justify-between items-center mb-4">
                ...
                {canEdit && (
                    <Button type="primary" icon={<PlusOutlined />} onClick={() => {
                        setEditBlog(null); blogForm.resetFields(); setModalOpen(true);
                    }}>Add Blog</Button>
                )}
            </div>
            <div className="flex flex-wrap gap-2 mb-4">
                ...
                >
                    ...
                    <Select.Option value="subtopic">Subtopic</Select.Option>
                </Select>
            ...
            <Collapse accordion>
                {blogs
                    ...
                    .sort((a, b) => {
                        ...
                        // For other fields: compare as string (case insensitive)
                    ...
                    .map(blog => (
                        <Panel header={
                            <div className="flex gap-4 items-center">
                                <img src={blog.imageIconurl} alt="cover" style={{ width: 50, height: 50, objectFit: 'cover' }} />
                                <div>
                            ...
                            <div className="mb-2 content-view"><b>Article Part 2:</b>
                                <div dangerouslySetInnerHTML={{ __html: blog.articlePart2 }} />
                            </div>
                            <div className="mb-2">
                                <b>Ad Title -</b>  {blog.advertisement?.title || '-'}
                                <div><b>Ad Description -</b>  {blog.advertisement?.description || '-'}</div>
                                <b>Ad URL -</b>  <a href={blog.advertisement?.url} target="_blank" rel="noreferrer">{blog.advertisement?.url}</a>
                            </div>
                            {canEdit && (
            ...
            {canEdit && (
                <Modal title={editBlog ? 'Edit Blog' : 'Add Blog'} open={modalOpen} onCancel={() => {
                    setModalOpen(false); setEditBlog(null); blogForm.resetFields();
                }} footer={null}>
                    <Form form={blogForm} layout="vertical" onFinish={handleBlogSubmit}>
                        ...
                        <Form.Item name="articlePart1" label="Article Part 1" rules={[{ required: true }]}>
                            <TiptapEditor />
                        </Form.Item>
                        <Form.Item name="articlePart2" label="Article Part 2" rules={[{ required: true }]}>
                            <TiptapEditor />
                        </Form.Item>
